...
with st.sidebar:
    ...
    st.markdown("👨‍💻 Made By: Gaurav Kumar Rao, Divyashree R, Chaitanya R")
...
def calculate_cancer_risk_score(age, gender, family_history, smoking, alcohol, diet, physical_activity,
                                environmental_exposure, medical_history):
    risk_score = 0
...
if st.session_state.page == "Cancer Risk Assessment":
    ...
    medical_history = st.selectbox("Do you have a personal medical history of cancer or related conditions?",
                                   ["No", "Yes"])
    ...
    if st.button("Assess Cancer Risk"):
        ...
        risk_score = calculate_cancer_risk_score(age, gender, family_history, smoking, alcohol, diet, physical_activity,
                                                 environmental_exposure, medical_history)
        ...
        st.markdown(
            f'<div class="message-container ai-message">Your estimated cancer risk level is: <strong>{risk_level}</strong></div>',
            unsafe_allow_html=True)
        st.markdown(
            f'<div class="message-container ai-message">Risk Percentage: <strong>{risk_percentage}</strong></div>',
            unsafe_allow_html=True)
        st.markdown(
            f'<div class="message-container ai-message">Note: This assessment provides a general risk estimation. For a comprehensive evaluation, please consult a healthcare professional.</div>',
            unsafe_allow_html=True)
...
if st.session_state.page == "Medical Assistant":
    ...
    if "message_log" not in st.session_state:
        st.session_state.message_log = [
            {"role": "ai", "content": "Hello! I'm your AI Medical Assistant. How can I help?"}]
    ...
    with chat_container:
        ...
        for message in st.session_state.message_log:
            ...
            st.markdown(f'<div class="message-container {message_class}">{message["content"]}</div>',
                        unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
...
# Health Risk Calculator Page
    ...
    if st.button("Calculate Risk"):
        ...
        risk_score = sum([bmi > 30, smoking == "Regularly", exercise == "Rarely", age > 50, blood_pressure == "Yes",
                          diabetes == "Yes"]) * 2
